...
class Place(HLAction):
    def __init__(self, lineno, hl_plan, env, robot, pos, obj, loc, gp):
        ...
        self.preconditions += [IsPDP(self.env, self, robot, 0, obj, gp, loc, self.traj, self.obj_traj)]
    ...
    def plot_consensus_pos(self):
        if not np.allclose(self.pos.get_value(), self.hl_pos.get_value()):
            place_pos = np.array(self.pos.get_value())
            place_pos[2] = 1
    ...
    def plot_consensus_obj_pos(self):
        if not np.allclose(self.loc.get_value(), self.hl_loc.get_value()):
    ...
    #     self.plot_consensus_obj_pos()
        ...
        # if not np.allclose(self.gp.get_value(), self.hl_gp.get_value()):
        #     hl_gp = np.array(self.hl_gp.get_value())
        #     # hl_gp[2] = 1

        #     place_pos = np.array(self.traj.get_value())
        #     place_pos[2] = 1

        #     hl_obj_pos = hl_gp + place_pos
        #     hl_obj_pos[2]=1
        #     place_obj_pos = np.array(self.obj_traj.get_value())
        #     place_obj_pos[2] = 1
